...
func TestModernUpdateSet(t *testing.T) {
	session := getModernSession(t)
	defer session.Close()

	c := session.DB("ikatago_test").C("accounts")
...
func TestModernFindSortLimitOne(t *testing.T) {
	session := getModernSession(t)
	defer session.Close()

	vc := session.DB("ikatago_test").C("accounts")
...
func TestModernUpdateId(t *testing.T) {
	session := getModernSession(t)
	defer session.Close()
	...
	err := c.UpdateId(account.ID, bson.M{"$set": account})
	if err != nil {
...
func TestModernAccountPipe(t *testing.T) {
	session := getModernSession(t)
	defer session.Close()
...
func TestModernRemove(t *testing.T) {
	session := getModernSession(t)
	defer session.Close()
...
func TestModernEnsureIndicesAccountService(t *testing.T) {
	session := getModernSession(t)
	defer session.Close()
